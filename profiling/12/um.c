...
static inline void addInstr(Segment_T seg, uint32_t instr)
{
	...
	int size = seg->size;
	if (size == length) {
		uint32_t* newInstrs = realloc(seg->instrs, sizeof(uint32_t) * size * 2);
		seg->size = size * 2;
		free(seg->instrs);
...
static inline void store_segment(Mem myMem, uint32_t ra, uint32_t rb, uint32_t val)
{
		...
		if (opcode == 13) {
			...
			switch(opcode) {
				case 0 :
					...
					break;
				case 1 :
					R[a] = ((myMem->segs)[R[b]]->instrs)[R[c]];
					break;
				case 2 :
					((myMem->segs)[R[a]]->instrs)[R[b]] = R[c];
					break;
				case 3 :
					add(a, b, c);
					break;
